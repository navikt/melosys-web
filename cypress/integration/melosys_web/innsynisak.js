describe.skip("Innsyn i sak", () => {
  beforeEach(() => {
    ...
    cy.visit("/");
    cy.get("#id-sokeskjema").type("17117802280").type("{enter}");
  });
  it("Sjekk antall Behandlinger ", () => {
    cy.get("a.behandling__link").should("have.length", 2);
  });
  it("Velger og klikker behandling med behandlingsstatus: opprettet", () => {
    ...
    cy.get("a.behandling__link").first().find("button").click();
    cy.get('div.skjemaelement [type="text"]').eq(1).type("02.02.2019").blur();
    cy.get("div.skjemaelement select").select("RETURNERT_NORGE").blur();
    cy.get("button.knapp.knapp--hoved").click();

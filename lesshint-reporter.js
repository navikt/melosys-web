...
function report(results) {
  const groupedResultsObject = results.reduce((acc, result) => {
    const group = acc[result.fullPath] || {
  ...
  groupedResults.forEach(group => {
    ...
    const NOF_ERRORS = group.results.filter(
      result => result.severity === 'error'
    ).length;
    const NOF_WARNINGS = group.results.filter(
      result => result.severity === 'warning'
    ).length;

    console.log(
      `${filepath} (${chalk.red(NOF_ERRORS)}/${chalk.yellow(NOF_WARNINGS)})`
    );

 ...
 */
const Inntekt = props => {
  const [visInntektTabell, setState] = useState(false);

  const toggleInntektTabellHandler = e => {
    e.preventDefault();
    setState(!visInntektTabell);
  };

  const { inntektListe } = props;
  ...
  const uuTabell = visInntektTabell ? (
    <div>
  ...
  const inntektInnhold = harMinstEnInntekt && (
    <div className="inntekt">
      <div className="inntekt__graf">
        ...
        <Nav.Knapp mini onClick={toggleInntektTabellHandler} className="vistabell__knapp">
          { visInntektTabell ? 'Skjul tabellen' : 'Vis grafen som tabell' }
        </Nav.Knapp>
      </div>
      {uuTabell}
    </div>

...
describe('LovvalgsbestemmelseStore oppdatering av eksisterende data', () => {
  ...
  beforeEach(() => {
    ...
    const bestemmelse = konverterLovvalgsbestemmelseTilStegData(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_1A);
    store.oppdaterStegData('TESTSTEG-1', bestemmelse);
  });

  it('Returnerer lovvalgsbestemmelse ved en lovvalgsbestemmelse', () => {
    expect(store.hent()).toBe(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_1A);
  });

  it('Returnerer siste verdi ved oppdatering av lovvalgsbestemmelse', () => {
    const bestemmelse = lagLovvalgsbestemmelse(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_2A);
    store.oppdaterStegData('TESTSTEG-1', bestemmelse);

    expect(store.hent()).toBe(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_2A);
  });
...
describe('LovvalgsbestemmelseStore slettData', () => {
  ...
  it('Sletter lovvalgsbestemmelse etter oppdatering', () => {
    store.oppdaterStegData('TESTSTEG-1', lagLovvalgsbestemmelse(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_2A));
    store.slettStegData('TESTSTEG-1');
    expect(store.hent()).toBeFalsy();
  });

  it('Returnerer verdien til andre steg ved sletting av første steg', () => {
    const bestemmelse = konverterLovvalgsbestemmelseTilStegData(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_2A);
    store.oppdaterStegData('TESTSTEG-2', bestemmelse);
    store.slettStegData('TESTSTEG-1');
    expect(store.hent()).toBe(MKV.Koder.lovvalgsbestemmelser.lovvalgbestemmelser_883_2004.FO_883_2004_ART13_2A);
  });

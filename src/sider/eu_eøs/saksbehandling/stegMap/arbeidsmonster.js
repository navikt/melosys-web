...
class Arbeidsmonster extends Steg {
  constructor(propsLight, stegPosisjon) {
    ...
    const offentligArbeidNorge =
      hentFaktaVerdi(offentligArbeidAntallLandFakta) === KV.Koder.OffentligArbeidAntallLand.NORGE_OG_ANNEN_VIRKSOMHET;
    const offentligArbeidAnnetLand =
      hentFaktaVerdi(offentligArbeidAntallLandFakta) ===
      KV.Koder.OffentligArbeidAntallLand.ANNET_LAND_OG_ANNEN_VIRKSOMHET;
    const offentligArbeidIFlereLand =
      hentFaktaVerdi(offentligArbeidAntallLandFakta) ===
      KV.Koder.OffentligArbeidAntallLand.FLERE_LAND_OG_ANNEN_VIRKSOMHET;
    ...
    const aktivitetINorgeNodvendig =
    ...
    this.kriterier = [
      {
        exec: () =>
          ...
          ((aktivitetINorge && erArbeidstaker) || loennetArbeidIFlereLand || offentligArbeidIFlereLand),
        nesteSteg: STEG.FORRETNINGSSTED,
      },
      {
        exec: () => aktivitetINorge && aktivitetNorgeUnder25Prosent && erSelvstendigNaeringsdrivende,
        nesteSteg: STEG.ARTIKKEL_13_2_B,
      },
      {
        exec: () =>
          aktivitetNorgeOver25Prosent && (erArbeidstaker || offentligArbeidIFlereLand || loennetArbeidIFlereLand),
        nesteSteg: STEG.ARTIKKEL_13_1_A_VEDTAK,
      },
      {
        exec: () => aktivitetNorgeOver25Prosent && erSelvstendigNaeringsdrivende,
        nesteSteg: STEG.ARTIKKEL_13_2_A_VEDTAK,
      },
      {
        exec: () => loennetArbeidINorge && erArbeidstakerOgSelvstendigNaeringsdrivende,
        nesteSteg: STEG.ARTIKKEL_13_3_VEDTAK,
      },
      {
        exec: () => harAvklaring && erArbeidstakerOgSelvstendigNaeringsdrivende && loennetArbeidIEttAnnetLand,

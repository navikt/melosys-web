...
export const delvisInnvilgelseSkjema = Yup.object().shape({
  fom: Yup.string().test(gyldigPeriodeTest).test(gyldigDatoTest("startdato")).required(STARTDATO_PAKREVD),
  tom: Yup.string().test(gyldigPeriodeTest).test(gyldigDatoTest("sluttdato")).required(SLUTTDATO_PAKREVD),
  fritekst: Yup.string().required(FRITEKST_PAKREVD),
});

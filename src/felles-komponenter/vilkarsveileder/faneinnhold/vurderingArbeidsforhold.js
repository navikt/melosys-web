...
const ArbeidsforholdLinje = ({ arbeidsforhold }) => (
  <div key={uuid()} className="arbeidsforhold__enkeltlinje">
    ...
    <button className="arbeidsforhold__enkeltlinje__knapp" onClick={() => {}}>-</button>
  </div>
);
...
const VurderingArbeidsforhold = props => {
  ...
  return (
    <div className="vurderingarbeidsforhold">
      ...
      <div>
        {arbeidsforholdene.map(arbeidsforhold => <ArbeidsforholdLinje key={uuid()} arbeidsforhold={arbeidsforhold} />)}
      </div>

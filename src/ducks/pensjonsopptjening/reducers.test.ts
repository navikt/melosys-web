...
describe("pensjonsopptjening reducer", () => {
  ...
  it("PENDING setter status til PENDING uten å røre data", () => {
    const state = reducer(
      { status: STATUS.OK, data: { inntektsAr: 2024, behandletAr: 2024, perioder: [] } },
      {
        type: Types.PENDING,
      },
    );
    expect(state.status).toBe(STATUS.PENDING);
    expect(state.data).toEqual({ inntektsAr: 2024, behandletAr: 2024, perioder: [] });
  });
  ...
  it("OK lagrer data og fjerner tidligere feil", () => {
    const respons = {
      inntektsAr: 2024,
      behandletAr: 2024,
      perioder: [{ aar: 2024, pgi: 100, kilde: "SKATT" as const }],
    };

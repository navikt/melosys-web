...
const mapStateToProps = (state: RootState) => ({
  ...
  medfolgendeEktefelle: behandlingsgrunnlagSelectors.MedfolgendeEktefelleSelector(state),
  });
...
const VurderingFamilie =
  ({
     bekreft, medfolgendeFamilie, medfolgendeBarn, medfolgendeEktefelle, redigerbart,
  }: Props & PropsFromRedux) => {
    const [skalBarnInnvilgesMedlemskap, setSkalBarnInnvilgesMedlemskap] = useState(new Map());
    const [skalEktefelleInnvilgesMedlemskap, setSkalEktefelleInnvilgesMedlemskap] = useState(new Map());
    ...
    const handleEktefelleInnvilgelseChange: ChangeEventHandler<HTMLInputElement> = (event) => {
      setSkalEktefelleInnvilgesMedlemskap(new Map(skalEktefelleInnvilgesMedlemskap.set(event.target.name, event.target.value)))
    };
    ...
    return (
      <div className="vurderingFamilie">
        <Nav.typo.Undertittel className="undertittel">Skal medfolgendeFamilie oppgitt i søknaden innvilges
          medlemskap?</Nav.typo.Undertittel>
        ...
        {(medfolgendeFamilie && medfolgendeFamilie.length > 0)
          ...
          <div>
            <Nav.Fieldset legend="Barn">
              <Nav.Row>
                {medfolgendeBarn.map((barn: MedfolgendeBarn) =>
                  <Nav.Column xs="6" key={barn.uuid}>
                    <Nav.typo.Normaltekst>{`${Utils.streng.storeForbokstaver(barn.navn)} (F.nr: ${barn.fnr})`}</Nav.typo.Normaltekst>
                    ...
                    {skalBarnInnvilgesMedlemskap.get(barn.uuid) === BOOLSK_STRING.USANN &&
                      <Nav.Select label="Begrunnelse:" onChange={handleBarnBegrunnelseChange}>
                        <option key="" value="" disabled={!redigerbart || valgtBegrunnelse.get(barn.uuid)}>Velg...</option>
              ...
              {medfolgendeBarn.some((barn: MedfolgendeBarn) => skalBarnInnvilgesMedlemskap.get(barn.uuid) === BOOLSK_STRING.USANN) &&
                <div>
            ...
            <Nav.Fieldset legend="Ektefelle/parner/samboer">
              <Nav.Row>
              {medfolgendeEktefelle.map((ektefelle: MedfolgendeBarn) =>
                  <Nav.Column xs="6" key={ektefelle.uuid}>
                    <Nav.typo.Normaltekst>{`${Utils.streng.storeForbokstaver(ektefelle.navn)} (F.nr: ${ektefelle.fnr})`}</Nav.typo.Normaltekst>
                    <Nav.Row className="familiemedlem_radio">
                      <Nav.Column xs="2">
                        <Nav.Radio
                          label="Ja"
                          ...
                          checked={skalEktefelleInnvilgesMedlemskap.get(ektefelle.uuid) === BOOLSK_STRING.SANN}
                          value={BOOLSK_STRING.SANN}
                          key={BOOLSK_STRING.SANN}
                          disabled={!redigerbart}
                        />
                      ...
                      <Nav.Column xs="2">
                        <Nav.Radio
                          ...
                          name={ektefelle.uuid}
                          onChange={handleEktefelleInnvilgelseChange}
                          checked={skalEktefelleInnvilgesMedlemskap.get(ektefelle.uuid) === BOOLSK_STRING.USANN}
                          value={BOOLSK_STRING.USANN}
                          key={BOOLSK_STRING.USANN}
                          disabled={!redigerbart}
                        />
                    ...
                    {skalEktefelleInnvilgesMedlemskap.get(ektefelle.uuid) === BOOLSK_STRING.USANN &&
                    <Nav.Select label="Begrunnelse:" onChange={handleEktefelleBegrunnelseChange}>
                      <option key="" value="" disabled={!redigerbart || valgtBegrunnelse.get(ektefelle.uuid)}>Velg...</option>
                    </Nav.Select>
                    }
                  </Nav.Column>
                )}
              </Nav.Row>
              {medfolgendeEktefelle.some((ektefelle: MedfolgendeBarn) => skalEktefelleInnvilgesMedlemskap.get(ektefelle.uuid) === BOOLSK_STRING.USANN) &&
              <div>

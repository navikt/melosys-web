...
function SoknadslandHandlingSlett(props) {
  ...
  const { soknadslandBegrunnelser, bekreft, avbryt, landkodeObjekt, redigerbart } = props;
  ...
  return (
    <div className="fjernland__linje">
      ...
      <div className="linje__knapper">
        <Nav.Knapp className="fjern__avbryt" disabled={!redigerbart} onClick={avbryt}>
          Avbryt
        </Nav.Knapp>
        <Nav.Knapp
          className="fjern__bekreft"
          disabled={!(redigerbart && begrunnelse !== "0")}
          onClick={() => bekreft(landkode, begrunnelse)}
        >
          Bekreft
        </Nav.Knapp>
      </div>

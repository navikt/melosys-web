...
class SoknadslandListe extends Component {
  ...
  bekreftFjern = (landkode, begrunnelseKode) => {
    if (
      begrunnelseKode === MKV.Koder.begrunnelser.opphold.FEIL_LAND_JOURNALFOERING ||
    ...
    this.props.oppdaterData(
      lagAvklartfakta(KV.Koder.SOKNADSLAND, landkode, KV.Koder.SoknadslandFaktaTyper.USANN, [begrunnelseKode])
    );
  };
  ...
  bekreftLeggTil = (landkode, begrunnelseKode) => {
    ...
    this.props.oppdaterData(
      lagAvklartfakta(KV.Koder.SOKNADSLAND, landkode, KV.Koder.SoknadslandFaktaTyper.SANN, [begrunnelseKode])
    );
  };
  ...
  angreFjern = (landkode) => {
    ...
    if (
      enkeltFakta.begrunnelseKoder.includes(MKV.Koder.begrunnelser.opphold.FEIL_LAND_JOURNALFOERING) ||
  ...
  render() {
    ...
    const { bekreftFjern, angreFjern, finnBegrunnelse, bekreftLeggTil, finnLandVedKode } = this;

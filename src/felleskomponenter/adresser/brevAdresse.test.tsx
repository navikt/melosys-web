...
describe("BrevAdresse", () => {
  ...
  it("skal rendre postnr og poststed", () => {
    const { container } = render(<BrevAdresse postnr="0001" poststed="Oslo" />);
  ...
  it("skal rendre region", () => {
    const { container } = render(<BrevAdresse region="Akershus" />);

    expect(container.textContent).toContain("Akershus");
  });

  it("skal rendre land", () => {
    const { container } = render(<BrevAdresse land="Norge" />);

    expect(container.textContent).toContain("Norge");
  });

  it("skal legge til custom className", () => {
    const { container } = render(<BrevAdresse className="custom-class" />);
  ...
  it("skal håndtere tom adresse", () => {
    const { container } = render(<BrevAdresse />);
  ...
  it("skal legge til komma etter adresselinjer", () => {
    const { container } = render(<BrevAdresse adresselinjer={["Storgata 1"]} postnr="0001" />);

    expect(container.textContent).toContain("Storgata 1,");
  });

  it("skal legge til komma etter postnr og poststed", () => {
    const { container } = render(<BrevAdresse postnr="0001" poststed="Oslo" region="Akershus" />);

    expect(container.textContent).toContain("0001 Oslo,");
  });

  it("skal legge til komma etter region", () => {
    const { container } = render(<BrevAdresse region="Akershus" land="Norge" />);
  ...
  it("skal håndtere tom adresselinjer array", () => {
    const { container } = render(<BrevAdresse adresselinjer={[]} postnr="0001" />);

    expect(container.textContent).toContain("0001");
  });

  it("skal håndtere én adresselinje", () => {
    const { container } = render(<BrevAdresse adresselinjer={["Storgata 1"]} />);

    expect(container.textContent).toContain("Storgata 1");
  });

  it("skal håndtere flere adresselinjer", () => {
    const { container } = render(<BrevAdresse adresselinjer={["Storgata 1", "Blokk A", "Leil. 12"]} />);
  ...
  it("skal håndtere partiell adresse med kun noen felter", () => {
    const { container } = render(<BrevAdresse adresselinjer={["Storgata 1"]} land="Norge" />);
  ...
  it("skal håndtere kun postnr uten poststed", () => {
    const { container } = render(<BrevAdresse postnr="0001" />);

    expect(container.textContent).toContain("0001");
  });

  it("skal håndtere kun poststed uten postnr", () => {
    const { container } = render(<BrevAdresse poststed="Oslo" />);

...
class VurderingSokkelSkip extends React.Component {
  ...
  avklartefaktaEndret = ({ value, id }, subjektID) => {
    const { oppdaterRessurs } = this.props;
    oppdaterRessurs(lagAvklartfakta(id, subjektID, value));
  };

  avklartefaktaBegrunnelseEndret = ({ value, id }, subjektID) => {
    const { oppdaterRessurs } = this.props;
    oppdaterRessurs(lagAvklartefaktaBegrunnelse(id, subjektID, value));
  };

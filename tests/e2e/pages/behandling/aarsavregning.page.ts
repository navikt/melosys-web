...
export class AarsavregningPage {
  ...
  async leggTilMedlemskapsperiode() {
    ...
    await expect(knapp, "Knapp 'Legg til periode' skal være synlig").toBeVisible({ timeout: 5000 });
    await knapp.click();
  ...
  async fyllUtMedlemskapsperiodeFomDato(index: number, dato: string) {
    ...
    await expect(trygdedekningDropdown, `Trygdedekning-dropdown for periode ${index + 1} skal være synlig`).toBeVisible(
      { timeout: 10000 },
    );
  ...
  async velgTrygdedekning(index: number, dekning: string) {
    ...
    await expect(select, `Trygdedekning-select for periode ${index + 1} skal være synlig`).toBeVisible({
      timeout: 10000,
    });
  ...
  async getAntallMedlemskapsperioder(): Promise<number> {
    ...
    await expect(firstSelect, "Første trygdedekning-select skal være synlig").toBeVisible({ timeout: 10000 });
  ...
  async fyllUtSkatteforholdFomDato(index: number, dato: string) {
    ...
    await expect(firstSkattLabel, "Label 'Skatteforhold' skal være synlig").toBeVisible({ timeout: 10000 });
  ...
  async velgSkatteplikttype(index: number, type: string) {
    ...
    await expect(radio, `Radio-button "${radioValue}" for skatteplikttype skal være synlig`).toBeVisible({
      timeout: 5000,
    });
  ...
  async fyllUtInntektsperiodeFomDato(index: number, dato: string) {
    ...
    await expect(firstInntektLabel, "Label 'Inntektsperiode' skal være synlig").toBeVisible({ timeout: 10000 });
  ...
  async velgKildetype(index: number, type: string) {
    ...
    await expect(select, `Kildetype-select for inntektsperiode ${index} skal være synlig`).toBeVisible({
      timeout: 10000,
    });
  ...
  async velgDeltGrunnlagJa() {
    ...
    await expect(jaRadio, "Radio-button 'Ja' skal være synlig").toBeVisible({ timeout: 10000 });
  ...
  async assertDatepickerErAktiv() {
    ...
    await expect(datepicker, "Datepicker-dialog skal være synlig").toBeVisible({ timeout: 2000 });
  }
  ...
  async assertValideringsfeilInneholder(tekst: string) {
    ...
    await expect(error, `Feilmelding som inneholder "${tekst}" skal være synlig`).toBeVisible();
  }
